...
long double ans, xi, yi, xii, yii, prev, vx, vy, dx, dy;
...
long double tsp(int u, int mask){
    ...
    for(int i=1;(1<<(i-1))<=mask;i++){
        ...
        int v = i;
        long double prev = tsp(v, mask^(1<<(i-1)));
        if(u==0){ans = min(ans, prev); continue;}
        xii = ar[u].sx+ar[u].vx*prev, yii = ar[u].sy+ar[u].vy*prev;
        xi= ar[v].sx+ar[v].vx*prev, yi = ar[v].sy+ar[v].vy*prev;
        dx=xii-xi, dy=yii-yi;
        vx = ar[u].vx, vy = ar[u].vy;
        long double a, b, c;
